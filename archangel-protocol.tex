...
Autonomous self-driving cars equipped with radar or lidar sensors and a camera system have a reaction time of 0.5 seconds. This means, the car continues to drive 7 metres without braking at a speed of 50 km/h. The Fraunhofer Institute for Reliability and Microintegration IZM, together with partners from industry (AVL, Jabil Optics, John Deere, etc.) is now developing a combined camera-radar module that can register changes in road traffic much faster: the module will have a reaction time of less than 10 milliseconds. It will react 50 times faster than conventional sensor systems and 160 times faster than humans, according to a study by the University of Michigan. The car thus only moves 15 cm without braking, which could prevent many accidents.
...
\section{Conclusion}
Even though self-driving cars don't have a long history, this branch of technology is certainly important and is far from its final form. From day-to-day the number of cars with self-driving capabilities are increasing. There are still lots of problems waiting to be solved because self-driving cars are an inevitable part of the future.

In our work, we managed to specify a new communication protocol which does pioneer work in protection of pedestrians. Our proposal clearly achieved superior safety level over any other self-driving car communication protocol by using the advantages of 5G cellular network such as its high bandwidth and low latency combined with our scoring system and unique package structure.

Thanks to our protocol, we managed to decrease self-driving cars average prediction time delay from 783 ms to 250 ms. We think our work is worth the effort since traffic safety is one of the most important problems but now it is one step closer to be solved.
In the future, we plan to address the following research lines:
\begin{itemize}
    \item We have a communication protocol of our proposal and we are going to release it after some simulations and tests.
    \item We are going to consider fine-tuning and customizing our protocol for different severe weather conditions.
\end{itemize}
...
\begin{thebibliography}{00}
    ...
    \bibitem{b6} Murphey, Y.L.; Liu, C.; Tayyab, M.; Narayan, D. Accurate pedestrian path prediction using neural networks. In Proceedings of the 2017 IEEE Symposium Series on Computational Intelligence (SSCI 2017), Honolulu, HI, USA, 27 November–1 December 2017; pp. 1–7.
\end{thebibliography}
\vspace{12pt}
